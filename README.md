...
* While waiting for the import script to run at work, put together a list of orchestras to scrape events for, focusing initially on the Northeast Corridor
  ...
  * New York
    ...
    * Second pass: Add university groups 
* Use pyspider to **quickly** build scrapers for the list above. Scrapers should run once every day, but no more than that. And they should be respectful by not hammering sites, using randomized sleeps to avoid that outcome
